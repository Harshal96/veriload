"""Authenticated object lifecycle scenario.

This example assumes a conventional JSON API:

- POST /auth/register creates a synthetic user.
- POST /auth/login returns a bearer access token.
- POST /objects creates an object owned by the authenticated user.
- GET /objects/{id} reads the object.
- DELETE /objects/{id} and DELETE /users/{id} clean up test data.
"""
...
class AuthenticatedObjectUser(VeriUser):
    """Authenticate each virtual user, exercise objects, and clean up later."""
    ...
    async def register_user(self) -> None:
        ...
        payload = {
            **profile,
            ...
            "company": {
                "id": self.persona.company.id,
                "name": self.persona.company.name,
                "industry": self.persona.company.industry,
            },
        }
    ...
    async def create_object_for_later_cleanup(self) -> None:
        response = await self.http.post(
            ...
            json={
                ...
                "body": {
                    ...
                    "industry": self.persona.job.industry,
                },
        ...
        self._remember(
            object_ids=(*self._object_ids(), object_id),
...
def _required_string(body: dict[str, Any], paths: tuple[str, ...]) -> str:
    for path in paths:
        ...
        if isinstance(value, int):
            return str(value)
    joined = ", ".join(paths)
    raise RuntimeError(f"Response did not include any of: {joined}")
